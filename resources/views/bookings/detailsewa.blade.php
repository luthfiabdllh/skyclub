...
<div x-data="calendar()" x-init="init()" class="flex justify-between my-12">
        ...
        <div class=" space-y-1">
            ...
            <div class="flex">
                <div class="flex items-center border rounded-lg px-2.5">
                    <p class="text-sm font-bold text-gray-900 dark:text-white" x-text="cek">4.95</p>
    ...
    <div class="border px-5 py-8 bg-white rounded-2xl max-h-fit space-y-7 sticky top-2" style="width: 490px">
        ...
        <template x-if="cart.length > 0">
            <ul class=" space-y-4">
                <template x-for="(item, index) in cart" :key="index">
                    <li class="w-full flex justify-between items-center border border-gray-500 text-lg rounded-xl py-2 px-4">
                        <div>
                            <p x-text="new Date(item.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })"></p>
        ...
        <div class="flex items-stretch space-x-8">
            ...
            <a href=""
                class="bg-red-600 w-full text-center py-3 rounded-lg font-bold text-white">Bayar</a>
            <a href=""
...
<script>
    function calendar() {
        return {
            ...
            init() {
                this.updateWeekDays();
                this.selectedDate = this.currentDate.toISOString().split('T')[0]; // Set tanggal saat ini sebagai default
                this.loadTimeSlots();
            },
